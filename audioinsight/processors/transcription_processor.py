...
class TranscriptionProcessor(BaseProcessor):
    ...
    async def _queue_parser_non_blocking(self, text_to_parse: str, speaker_info):
        """Queue sentence parser request without blocking transcription processing."""
        try:
            if self.coordinator and self.coordinator.transcript_parser:
                # No more length validation - directly queue for sentence processing
                success = await self.coordinator.transcript_parser.queue_parsing_request(text_to_parse, speaker_info, None)
                if success:
                    logger.debug(f"✅ Sentence parser queue accepted {len(text_to_parse)} chars")
                else:
                    logger.debug(f"⏳ Sentence parser queue busy, will retry later")
        except Exception as e:
            logger.debug(f"Sentence parser queue error (non-critical): {e}")
